...
An office-hour demo of a true multi-agent system that automatically processes meeting minutes, extracts action items, performs legal research, and emails results to the relevant team members — across two separate machines on the same local network.

This builds on an earlier project that only extracted tasks. Here, a second AI agent performs additional documentation research for legal tasks before sending the final email.
